...
// };
  ...
  //       const pageCanvasHeight = Math.floor((pageHeight - 2 * margin) * canvas.width / imgWidth);
  ...
  //         // Check if the remaining content fits on one page
  //         const sliceHeight = remainingHeight <= pageCanvasHeight ? remainingHeight : pageCanvasHeight;
  ...
  //         // Add a new page only if there is more content to process
  ...
  const downloadPDF = () => {
    ...
    html2canvas(element, { 
      ...
      try {
        ...
        // If the entire content fits on one page, add it in one go
        if (imgHeight <= pageHeight - 2 * margin) {
          pdf.addImage(canvas, 'PNG', margin, margin, imgWidth, imgHeight);
        } else {
          // If content is larger than one page, use multi-page approach
          const pageCanvasHeight = Math.floor((pageHeight - 2 * margin) * canvas.width / imgWidth);
          let position = 0;
          ...
          while (position < canvas.height) {
            ...
            // If this is the first page or not the last slice, add the image
            pdf.addImage(imgData, 'PNG', margin, margin, imgWidth, sliceImgHeight);
  
            position += sliceHeight;
  
            // Add a new page only if there's more content and it's not the last slice
